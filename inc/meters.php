...
public function displayMeterCard($uid = null) {
	...
	$output  = "<div class=\"col-md-4\">";
		$output .= "<div class=\"card mb-4 shadow-sm\" >";
			...
			$output .= "<div class=\"card-img-overlay\"><h5 class=\"card-title\">" . $meter['name'] . "</h5><p class=\"card-text\"><samp>" . $meter['serial'] . "</samp></p></div>";
			$output .= "<div class=\"card-body\">";
				...
				if ($this->daysSinceLastUpdate($meter['uid']) > 80) {
					$output .= "<span class=\"badge badge-danger\">Updated " . $this->daysSinceLastUpdate($meter['uid']) . " days ago</span>";
				} else if ($this->daysSinceLastUpdate($meter['uid']) > 40) {
					$output .= "<span class=\"badge badge-warning\">Updated " . $this->daysSinceLastUpdate($meter['uid']) . " days ago</span>";
				} else if ($this->daysSinceLastUpdate($meter['uid']) < 30) {
					$output .= "<span class=\"badge badge-primary\">Updated " . $this->daysSinceLastUpdate($meter['uid']) . " days ago</span>";
				}
			$output .= "</div>";	
		$output .= "</div>";
	$output .= "</div>";
