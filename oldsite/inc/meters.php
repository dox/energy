...
public function meterBadge($uid = null) {
	$this->meterUID = $uid;
	$meter = $this->getOne();
	...
	$output = "<span class=\"badge " . $class . "\">" . $this->type . "</span>";
...
public function displayMeterCard($uid = null) {
	...
	if ($this->daysSinceLastUpdate($meter['uid']) > 80) {
		$lastUpdated = "<span class=\"badge bg-danger\">" . $text . "</span>";
	} else if ($this->daysSinceLastUpdate($meter['uid']) > 40) {
		$lastUpdated = "<span class=\"badge bg-warning\">" . $text . "</span>";
	} else if ($this->daysSinceLastUpdate($meter['uid']) < 30 && $this->daysSinceLastUpdate($meter['uid']) > 0) {
		$lastUpdated = "<span class=\"badge bg-primary\">" . $text . "</span>";
	}	else if ($this->daysSinceLastUpdate($meter['uid']) == 0) {
	 $lastUpdated = "<span class=\"badge bg-success\">Updated Today</span>";
 } else {
		$lastUpdated = "<span class=\"badge bg-primary\">No Readings Yet</span>";
	}

	$output  = "<div class=\"card\" style=\"width: 18rem;\">";
	if (isset($meter['photograph'])) {
		$img = "uploads/" . $meter['photograph'];
		$output .= "<img class=\"card-img-top\" src=\"" . $img . "\" width=\"100%\" height=\"225\" alt=\"" . $meter['name'] . "\">";
	} else {
		$output .= "<svg class=\"card-img-top\" width=\"100%\" height=\"225\" xmlns=\"http://www.w3.org/2000/svg\" preserveAspectRatio=\"xMidYMid slice\" focusable=\"false\" role=\"img\" aria-label=\"No image available\"\"><rect width=\"100%\" height=\"100%\" fill=\"#55595c\"/></svg>";
	}
